package glorydark.CloudInventory.command;
...
import glorydark.CloudInventory.gui.FormFactory;
    ...
    public boolean execute(CommandSender commandSender, String s, String[] strings) {
        if (strings.length < 1) {
            if (commandSender instanceof Player) {
                FormFactory.showMainMenu(((Player) commandSender).getPlayer());
            } else {
        ...
        switch (strings[0]) {
            case "savenbt":
                ...
                Config shopcfg = new Config(CloudInventoryMain.plugin.getDataFolder() + "/nbtsave.yml", Config.YAML);
            ...
            case "giveall":
                if (commandSender instanceof Player) {
                    Lang.getTranslation("Message.UseInConsoleOnly");
                    return true;
                }
                if (strings.length < 2) {
                    return false;
            ...
            case "give": //yun give xx id meta amount
                ...
                if (commandSender instanceof Player) {
                    if (commandSender.isOp()) {
                        ...
                        if (!player.isOnline()) {
                            commandSender.sendMessage(Lang.getTranslation("GiveItemFailed"));
                            return true;
                        }
            ...
            case "q":
                if (commandSender instanceof Player) {
                    CloudBag.withdrawPlayerBagItem(Integer.parseInt(strings[1]), (Player) commandSender);
                } else {
                    commandSender.sendMessage(Lang.getTranslation("Message.UseInGameOnly"));
                }
                break;
