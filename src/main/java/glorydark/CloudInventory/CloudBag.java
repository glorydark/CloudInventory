...
import glorydark.CloudInventory.util.Inventory;
...
public class CloudBag {
    ...
    public static List<String> getPlayerBag(Player player) {
        Config config = new Config(CloudInventoryMain.plugin.getDataFolder() + "/players/" + player.getName() + ".yml");
        return new ArrayList<>(config.getStringList("Inventory"));
    }

    public static void withdrawPlayerBagItem(Integer index, Player player) {
        Config config = new Config(CloudInventoryMain.plugin.getDataFolder() + "/players/" + player.getName() + ".yml");
    ...
    public static Boolean addItemToCloud(Player player, Item item) {
        if (item.getId() == 0) {
            player.sendMessage(Lang.getTranslation("Message.ForbidAirBeingUpload"));
            return false;
        }
        Config config = new Config(CloudInventoryMain.plugin.getDataFolder() + "/players/" + player.getName() + ".yml");
        ...
        if (arrayList.size() >= getMaxCloudSlot(player.getName())) {
            return false;
        }
    ...
    public static Integer getMaxCloudSlot(String p) {
        ...
        if (config.exists("InventorySlots")) {
            ...
            config.set("InventorySlots", CloudInventoryMain.defaultMaxSlot);
            config.save();
            return CloudInventoryMain.defaultMaxSlot;
        }
    }

    public static void setMaxCloudSlot(String p, int count) {
        Config config = new Config(CloudInventoryMain.plugin.getDataFolder() + "/players/" + p + ".yml");
